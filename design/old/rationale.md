...
export function redraw() {
	for (let {o, func} of subtrees.values()) {
		o.next(config(func))
	}
}
```
...
- Hook composition is as simple as a procedural function call. No, really - it's just `let result = useHook(...args)`. Similarly, decomposition is equally simple.
