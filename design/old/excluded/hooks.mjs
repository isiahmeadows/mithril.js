...
export let IGNORE = new Error("Hooks.IGNORE")

function run(state, paramStream) {
    let prevState = currentState, prevIndex = currentIndex
...
function getScheduled(state) {
    ...
    let scheduled = state.scheduled = {effects: [], updateView: false}
    function callback() {
        state.scheduled = undefined
        for (let {index, handler} of scheduled.effects) {
...
function initHooks(paramStream, body) {
    ...
    return createStream((observer) => {
        ...
        let subscription = run(state, paramStream)
        return () => {
            try {
                for (let {done} of state.slots) {
...
export function useStreamFactory(factory) {
    ...
    if (slot.factory !== factory) {
        ...
        if (prev != null) prev()
        try {
            let sub = factory(createStream((o) => {
